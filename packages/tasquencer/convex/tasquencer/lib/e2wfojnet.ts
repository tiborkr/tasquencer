...
import { Marking } from '../elements/marking'
...
export class E2WFOJNet {
  ...
  orJoinEnabled(M: Marking, orJoin: BaseTask): boolean {
    ...
    // An OR-join with no input conditions cannot be enabled
    // This is a structural issue - OR-joins must have at least one input
    if (X.size === 0) {
      return false
    }
