...
it("handles workflow completion in nested workflows", async ({ expect }) => {
  ...
  expect(workflowsT3_2[0].state).toBe("completed");
  ...
  expect(completedTasks.map((t) => t.name)).toEqual(["t1", "t2", "t3"]);
  ...
  expect(enabledTasks3[0].name).toBe("t4");

  const workItemsT4 = await t.query(
    internal.testing.tasquencer.getWorkflowTaskWorkItems,
    {
      workflowId: id,
      taskName: "t4",
  ...
  expect(workItemsT4.length).toBe(1);
  expect(workItemsT4[0].state).toBe("initialized");

  await t.mutation(internal.testing.tasquencer.startWorkItem, {
    workflowName: "main",
    workItemId: workItemsT4[0]._id,
    workflowVersionName: WORKFLOW_VERSION_NAME,
  });
  ...
  expect(completedTasks2.length).toBe(4);
  expect(completedTasks2.map((t) => t.name)).toEqual(["t1", "t2", "t3", "t4"]);
