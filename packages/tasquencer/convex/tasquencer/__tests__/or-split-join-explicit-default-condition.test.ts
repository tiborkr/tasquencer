      ...
      to
        ...
        .route(async ({ route }) => {
          const routes: AvailableRoutes<typeof route>[] = [
            route.toCondition("register->book_hotel"),
          ];
...
it('runs a net with "or" split and "or" join when having explicit default condition (1)', async ({
  expect,
}) => {
...
it('runs a net with "or" split and "or" join when having explicit default condition (2)', async ({
  expect,
}) => {
  await withVersionManagerBuilders(
    {
