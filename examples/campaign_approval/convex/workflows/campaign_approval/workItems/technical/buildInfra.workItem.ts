...
/**
 * buildInfra work item - Set up landing pages, email templates, audience segments
 * Required scope: campaign:ops
 *
 * Part of Phase 5 Technical Setup (parallel task).
 * Runs in parallel with configAnalytics and setupMedia.
 */

const buildInfraPolicy = authService.policies.requireScope('campaign:ops')

const buildInfraActions = authService.builders.workItemActions
  .start(z.never(), buildInfraPolicy, async ({ mutationCtx, workItem }) => {
    ...
    async ({ mutationCtx, workItem, parent }, payload) => {
      ...
      if (metadata) {
        await mutationCtx.db.patch(metadata._id, {
          payload: {
            type: 'buildInfra' as const,
            taskName: 'Build Infrastructure',
            infraReady: payload.infraReady,
          },
        })
      }
    },
...
export const buildInfraWorkItem = Builder.workItem('buildInfra').withActions(
  buildInfraActions.build(),
)

/**
 * buildInfra task - Parallel task in Phase 5 Technical Setup
 */
export const buildInfraTask = Builder.task(buildInfraWorkItem).withActivities({
