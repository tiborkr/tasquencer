...
const logMaterialsExpensePolicy = authService.policies.requireScope('dealToDelivery:expenses:create')

// Schema for logging a materials expense
const logMaterialsExpensePayloadSchema = z.object({
  projectId: z.string(),
  description: z.string().min(1, 'Description is required'),
  amount: z.number().positive('Amount must be positive'), // Amount in cents
  currency: z.string().default('USD'),
  date: z.number(), // Unix timestamp
  vendorName: z.string().optional(), // e.g., office supply store
})

const logMaterialsExpenseActions = authService.builders.workItemActions
  .start(z.never(), logMaterialsExpensePolicy, async ({ mutationCtx, workItem }) => {
  ...
  .complete(
    logMaterialsExpensePayloadSchema,
    logMaterialsExpensePolicy,
    async ({ mutationCtx, workItem }, payload) => {
      ...
      await mutationCtx.db.patch(metadata!._id, {
        payload: {
          type: 'logMaterialsExpense' as const,
...
export const logMaterialsExpenseWorkItem = Builder.workItem('logMaterialsExpense')
  .withActions(logMaterialsExpenseActions.build())
