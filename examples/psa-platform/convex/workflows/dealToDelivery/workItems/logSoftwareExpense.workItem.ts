...
const logSoftwareExpensePolicy = authService.policies.requireScope('dealToDelivery:expenses:create')

// Schema for logging a software expense
const logSoftwareExpensePayloadSchema = z.object({
  projectId: z.string(),
...
const logSoftwareExpenseActions = authService.builders.workItemActions
  .start(z.never(), logSoftwareExpensePolicy, async ({ mutationCtx, workItem }) => {
  ...
  .complete(
    logSoftwareExpensePayloadSchema,
    logSoftwareExpensePolicy,
    async ({ mutationCtx, workItem }, payload) => {
      ...
      await mutationCtx.db.patch(metadata!._id, {
        payload: {
          type: 'logSoftwareExpense' as const,
...
export const logSoftwareExpenseWorkItem = Builder.workItem('logSoftwareExpense')
  .withActions(logSoftwareExpenseActions.build())
