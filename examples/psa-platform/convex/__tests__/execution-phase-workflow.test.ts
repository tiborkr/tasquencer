...
/**
 * NOTE: Tests for MonitorBudgetBurn, Budget Overrun Flow, PauseWork, RequestChangeOrder,
 * and GetChangeOrderApproval work items require complex workflow progression because of
 * the execution phase's parallel topology:
 *
 ...
 *
 * To properly test these work items, we would need to:
 * 1. Complete createAndAssignTasks
 * 2. Complete at least one of the parallel tasks (trackTime, trackExpenses, executeProjectWork)
 * 3. Wait for reviewExecution to complete
 * 4. Then monitorBudgetBurn becomes enabled
 * 5. Complete monitorBudgetBurn with budgetOk=false to enable pauseWork
 * 3. Then pauseWork becomes enabled
 * 4. Complete pauseWork to enable requestChangeOrder
 * 5. Complete requestChangeOrder to enable getChangeOrderApproval
 *
 * The domain logic for these work items is already tested in the Budget Overrun Flow test above,
 * which verifies the routing works correctly. Individual work item unit tests would require
 * completing the full workflow path, which is expensive in terms of test execution time.
 *
 * TODO: Add comprehensive end-to-end tests for the full budget overrun → change order flow
 * Reference: .review/recipes/psa-platform/specs/06-workflow-execution-phase.md
 */
