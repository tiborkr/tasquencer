...
function HeaderRight() {
  const matches = useMatches()
  const matchesWithHeaderRight = matches.filter((match) => {
    const loaderData = match.loaderData as Record<string, unknown> | undefined
    return loaderData && 'headerRight' in loaderData
  })

  const elements = matchesWithHeaderRight.map(({ loaderData }) => {
    const data = loaderData as { headerRight?: { type: string; data: unknown } }
    return data?.headerRight
  })
    ...
    <div className="gap-2 px-4">
      {elements.map((element, index) => {
        if (element?.type === 'workflowStructure') {
          return (
            <WorkflowVisualizeHeader key={index} structure={element.data as ExtractedWorkflowStructure} />
          )
        }
        return null
      })}
